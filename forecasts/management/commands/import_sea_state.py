...
from forecasts.models import SeaState
...
class Command(BaseCommand):
    ...
    help = "Sync SeaState records from the legacy WIMP2 database."

    def add_arguments(self, parser):
        parser.add_argument("--replace", action="store_true", help="Delete existing SeaState records before importing.")
    ...
    def handle(self, *args, **options):
        ...
        with connections["legacy"].cursor() as cursor:
            cursor.execute("""
                SELECT * FROM val_sea_state WHERE description IS NOT NULL
            """)
        ...
        with transaction.atomic():
            ...
            if options["replace"]:
                deleted_count, _ = SeaState.objects.all().delete()
            ...
            existing_ids = set(
                SeaState.objects.filter(id__in=[record.id for record in records]).values_list("id", flat=True)
            )
            ...
            if new_records:
                SeaState.objects.bulk_create(new_records, batch_size = batch_size)

            # UPDATE EXISTING RECORDS ONLY IF SOURCE CHANGED
            existing_objects = {
                obj.id: obj
                for obj in SeaState.objects.filter(id__in=existing_ids)
            }
            ...
            if existing_records:
                ...
                field_names = [
                    field.name
                    for field in SeaState._meta.fields
                    if field.name != "id"
                ]

                SeaState.objects.bulk_update(existing_records, field_names, batch_size=batch_size,)
